...
getVaccinations(): Observable<IVaccinations[]> {
    
    let url: string = this.apiUrl + 'spDiseases/getAll' ;
    return this._httpClient.get<IVaccinations[]>(url).catch(this.handleError);
  }

  getFormSeucirty(frmName:string): Observable<IFormSecurity> {
   
    let url: string = this.apiUrl + 'spDiseases/GetFormSecurity?formName='+frmName ;
  ...
  updateRecord(spDiseases: IVaccinations): Observable<boolean> {
    
    let url = this.apiUrl + 'spDiseases/postDepartment';
    return this._httpClient.post<boolean>(url, spDiseases).catch(this.handleError);
  }
  
  deleteRecord(spDiseases: IVaccinations):Observable<boolean>{
    
    let url = this.apiUrl + 'spDiseases/Delete';
    return this._httpClient.post<boolean>(url,spDiseases).catch(this.handleError);
  }
