...
export class StatusesService {
  ...
  getStatuses(): Observable<IStatuses[]> {
    debugger;
    let url: string = this.apiUrl + 'spStatuses/getAll' ;
    return this._httpClient.get<IStatuses[]>(url).catch(this.handleError);
  }

  getFormSeucirty(frmName:string): Observable<IFormSecurity> {
   
    let url: string = this.apiUrl + 'spStatuses/GetFormSecurity?formName='+frmName ;
  ...
  updateRecord(spStatuses: IStatuses): Observable<boolean> {
    
    let url = this.apiUrl + 'spStatuses/postDepartment';
    return this._httpClient.post<boolean>(url, spStatuses).catch(this.handleError);
  }
  
  deleteRecord(spStatuses: IStatuses):Observable<boolean>{
    
    let url = this.apiUrl + 'spStatuses/Delete';
    return this._httpClient.post<boolean>(url,spStatuses).catch(this.handleError);
  }
