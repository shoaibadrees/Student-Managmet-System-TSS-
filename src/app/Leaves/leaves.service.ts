...
export class LeaveService {
  ...
  getDepartments(): Observable<IPayLeaves[]> {
    let url: string = this.apiUrl + 'PayLeaves/getAll' ;
    return this._httpClient.get<IPayLeaves[]>(url).catch(this.handleError);
  }

  getFormSeucirty(frmName:string): Observable<IFormSecurity> {
    
    let url: string = this.apiUrl + 'PayLeaves/GetFormSecurity?formName='+frmName ;
  ...
  updateRecord(PayDepartment: IPayLeaves): Observable<boolean> {
    let url = this.apiUrl + 'PayLeaves/postShifts';
    return this._httpClient.post<boolean>(url, PayDepartment).catch(this.handleError);
  }
  
  deleteRecord(payDeparmtent: IPayLeaves):Observable<boolean>{
    let url = this.apiUrl + 'PayLeaves/Delete';
