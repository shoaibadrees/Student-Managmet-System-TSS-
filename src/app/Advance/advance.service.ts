...
export class AdvanceService {
  ...
  getDepartments(): Observable<IPayAdvance[]> {
    let url: string = this.apiUrl + 'PayShifts/getAll' ;
    return this._httpClient.get<IPayAdvance[]>(url).catch(this.handleError);
  }

  getFormSeucirty(frmName:string): Observable<IFormSecurity> {
    debugger;
    let url: string = this.apiUrl + 'PayShifts/GetFormSecurity?formName='+frmName ;
  ...
  updateRecord(PayDepartment: IPayShifts): Observable<boolean> {
    let url = this.apiUrl + 'PayShifts/postShifts';
    return this._httpClient.post<boolean>(url, PayDepartment).catch(this.handleError);
  }
  
  deleteRecord(payDeparmtent: IPayShifts):Observable<boolean>{
    let url = this.apiUrl + 'PayShifts/Delete';
