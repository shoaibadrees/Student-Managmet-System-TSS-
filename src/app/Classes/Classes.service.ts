...
export class ClassesService {
  ...
  getClasses(): Observable<IClasses[]> {
    let url: string = this.apiUrl + 'spClasses/getAll' ;
    return this._httpClient.get<IClasses[]>(url).catch(this.handleError);
  }

  getFormSeucirty(frmName:string): Observable<IFormSecurity> {
   
    let url: string = this.apiUrl + 'spClasses/GetFormSecurity?formName='+frmName ;
  ...
  updateRecord(spClasses: IClasses): Observable<boolean> {
    
    let url = this.apiUrl + 'spClasses/postDepartment';
    return this._httpClient.post<boolean>(url, spClasses).catch(this.handleError);
  }
  
  deleteRecord(spClasses: IClasses):Observable<boolean>{
    
    let url = this.apiUrl + 'spClasses/Delete';
    return this._httpClient.post<boolean>(url,spClasses).catch(this.handleError);
  }
