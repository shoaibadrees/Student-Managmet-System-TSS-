...
@Injectable()
export class spStudentParentInfosService {
  ...
  getStudentParentInfoOnId(studentID:number):Observable<IspStudentParentInfos[]>{
    
    let url: string = this.apiUrl + 'spStudentParentInfos/getStuentParentInfoOnID?studentID='+studentID;
    
    return this._httpClient.get<IspStudentParentInfos[]>(url).catch(this.handleError);
  }
 
  insertOrUpdateRecord(spStudentParentInfos: IspStudentParentInfos[],studentID:number): Observable<number> {
    let url = this.apiUrl + 'spStudentParentInfos/postStudentParentInfos?studentID='+studentID;
    return this._httpClient.post<number>(url, spStudentParentInfos).catch(this.handleError);
  }
  getFormSeucirty(frmName:string): Observable<IFormSecurity> {

    let url: string = this.apiUrl + 'spStudentParentInfos/GetFormSecurity?formName='+frmName ;
